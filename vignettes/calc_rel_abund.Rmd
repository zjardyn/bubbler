...
```
Whether you choose to scale your plots or leave them as true abundance will depend on the purpose of the graphs. There are no hard-fast rules and you may run into both situations where you need scaled and unscaled relative abundances. 
...
If you want the best of both worlds, you can create a scaled bargraph, and superimpose the proportional abundances. To do this, we create two relative abundance tables. The code becomes a little involved because we are also splitting by antibiotic usage.  
...
# only the rows where antibiotic usage is no, same for yes
no_unscaled <- q %>%
    filter(reported_antibiotic_usage == "No")
yes_unscaled <- q %>%
    filter(reported_antibiotic_usage == "Yes")

# we need to generate the points by summing across the grouped variable
points_no <- no_unscaled %>%
    group_by(body_site, reported_antibiotic_usage) %>%
    summarise(sum = sum(rel_abund), .groups = "drop" ) 

points_yes <- yes_unscaled %>%
    group_by(body_site, reported_antibiotic_usage) %>%
    summarise(sum = sum(rel_abund), .groups = "drop" ) 
...
q2 <- rel_abund_qiime(
    ...
    metadata_qiime = metadata_q,
    taxa_level = "Genus",
    var = "body_site") %>% # grouping variable
    pool_taxa(n_taxa = 8, keep_metadata = TRUE) %>%
    arrange_taxa()
...
p1 <- q2 %>%
    filter(reported_antibiotic_usage == "No") %>%
    bar_plot(x_var = "body_site", position = "fill") +
    # ggplot(aes(x = body_site, y = rel_abund, fill = taxon)) +
    # geom_bar(stat = "identity", position = "fill") +
    geom_point(data = points_no, aes(x = body_site, y = sum, color = " "),
               inherit.aes = FALSE) +
    geom_line(data = points_no, aes(x = body_site, y = sum, group = 1, color = " "),
               inherit.aes = FALSE) + scale_color_manual(values = colors) +
    labs(x = "Body Site",
         y = "Mean relative abundance",
         color = "X-var relative abundance") + 
    scale_fill_viridis_d(option = "turbo")
...
p2 <- q2 %>%
    ...
    ggplot(aes(x = body_site, y = rel_abund, fill = taxon)) +
    geom_bar(stat = "identity", position = "fill") +
    geom_point(data = points_yes, aes(x = body_site, y = sum),
               inherit.aes = FALSE) +
    geom_line(data = points_yes, aes(x = body_site, y = sum, group = 1),
               inherit.aes = FALSE) +
    labs(x = "Body Site",
         y = "Mean relative abundance" ) + 
    scale_fill_viridis_d(option = "turbo")
